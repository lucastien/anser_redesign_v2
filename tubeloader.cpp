...
TubeLoader::TubeLoader(TubeData *tube, const QString& tubeFile):
    m_tube(tube),
    m_tubeFile(tubeFile)
{
...
bool TubeLoader::loadTube(const QString& suff)
{
    ...
    for(; it != m_tube->channels.end(); it++){
        ...
        ch->setDataSetId(QString::number(year)+suff);
...
int TubeLoader::sizeHeader(TubeHeader *hdr, QDataStream &in)
{
    int count;
    int size;
    char * buff = (char *)hdr;
...
int TubeLoader::initTube(const int npt)
{
...
int TubeLoader::formatBuff(char *buff, int nbyte)
{
    int n=0;
    short *p,*q;
    ...
    while(p < q)
    {
        ...
        if(m_tube->channels.find(ch) == m_tube->channels.end()){
            ...
            ChannelPtr chanObj(new Channel(QString::number(m_tube->hdr.chanOrder[ch]))); //Temporarily set channel name equals to chanOrder
            chanObj->setFullName(fullName);
            chanObj->setFreq(m_tube->hdr.test[freq_idx].freq);
            chanObj->add(x, y);
...
void TubeLoader::fixHdr(TubeHeader *h)
{
    int i=0;
